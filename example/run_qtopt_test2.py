...
def load(pol, targ_qf1, targ_qf2, qf, optim_qf, off_traj):
	...
	#optim_qf = torch.optim.Adam(qf_net.parameters(), args.qf_lr)
        ...
        while args.max_epis > total_epi:
            ...
            with measure('train'):
                ...
                if x > 1:
                    if is_initialized :
                         load(pol, targ_qf1, targ_qf2, qf, optim_qf, off_traj)
                    on_traj = Traj(traj_device='cpu')
                    on_traj.add_epis(epis)

                    on_traj = ef.add_next_obs(on_traj)
                    on_traj.register_epis()
                    with open('garbage/qt_opt_models/on_traj' + str(x) + '.pkl', 'wb') as output:
            	        pickle.dump(on_traj, output, pickle.HIGHEST_PROTOCOL)
                    del on_traj
                    is_initialized = True
                else:
                    if is_initialized :
                         load(pol, targ_qf1, targ_qf2, qf, optim_qf, off_traj)
                    with open('garbage/qt_opt_models/on_traj' + str(2) + '.pkl', 'rb') as input:
                      		on_traj2 = pickle.load(input)
                    on_traj = Traj(traj_device='cpu')
                    on_traj.add_epis(epis)
                    ...
                    with open('garbage/qt_opt_models/company_data.pkl', 'wb') as output:
